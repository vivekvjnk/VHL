...
When the agent is constructed, `model_post_init` fires and attempts to load the snapshot. The loading follows a priority chain:

```
replay_mode = True?
    │
    ├─► replay_persistence set?
    │       └─► Load EventLog from LocalFileStore(persistence_dir)
    │           (reads the on-disk event journal written by a previous run)
    │
...
step(conversation, on_event)
    ...
    │       ├─► Reconstruct typed Action from tool_call.arguments
    │       │   (uses tool.action_from_arguments → validates against schema)
...
> **Key design note:** The action is always *reconstructed* from the recorded `tool_call.arguments` via the live tool's `action_from_arguments()`. This validates the arguments against the *current* tool schema, catching schema drift automatically.
...
### 4.3 Transparent Schema Validation During Replay

Rather than injecting the raw recorded `Action` object, the replay reconstructs each action by parsing `tool_call.arguments` through `tool.action_from_arguments()`. This means **any schema drift in the tool's `Action` model is caught immediately** during replay, before the tool even executes.

### 4.4 Parallel Tool Execution Preserved
